...
Public Class main_pos_system
    ...
    Private v_DeliveryId As String = ""
    ...
    'Private v_DisplayForm As frm_screen_user
    ' ⭐ NEW: Public Sub to receive data from frm_dl_list ⭐
    Public Sub SetDeliveryInfo(ByVal deliveryId As String, ByVal deliveryName As String)
    ...
    Private Async Sub btnLogout_Click(sender As Object, e As EventArgs) Handles btnLogout.Click
        ...
        ' 2.1. Get User Data
        Dim userIdToLogout As String = LoggedInUserId.Trim()
        Dim userNameForAudit As String = LoggedInUserName.Trim()

        If String.IsNullOrEmpty(userIdToLogout) Then
            MessageBox.Show("User session data is missing. Exiting application.", "Audit Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Application.Exit()
            Exit Sub
        End If

        ' Disable UI elements during network operation
        ' You may need to replace 'Guna.UI2.WinForms.Guna2Button' with your actual button type
        Dim btn = TryCast(sender, Guna.UI2.WinForms.Guna2Button)
        If btn IsNot Nothing Then btn.Enabled = False
        Me.Cursor = Cursors.WaitCursor

        Try
            ' 2.2. Construct JSON Payload
            ...
            Using client As New HttpClient()
                ...
                ' 2.3. Perform Server-Side Logout Audit
                Dim response As HttpResponseMessage =
                    Await client.PostAsync($"{API_BASE_URL}/api/auth/logout", content)

                If Not response.IsSuccessStatusCode Then
                    ' Log failure but proceed with local logout/exit
                    Dim body = Await response.Content.ReadAsStringAsync()
                    MessageBox.Show(
                        $"Warning: Logout audit failed. Status: {response.StatusCode}. Details: {body}",
                        "Audit Failure",
        ...
        Catch ex As Exception
            MessageBox.Show($"Network Error during logout audit: {ex.Message}", "System Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        Finally
            ' 2.4. Local Logout & Form Transition
            If Not Me.IsDisposed Then

                Me.Hide()

                ' Clear the shared user data
                LoggedInUserId = ""
                LoggedInUserName = ""
    ...
    Private Const API_BASE_URL As String = "http://172.29.29.56:5005" ' Your Node.js API server address
    ...
    Public Property StoreNameOnly As String
    Public Property CashierId As String
    Public Property CashierName As String
    Public Property LoggedInUserId As String

    Public Sub SetUserData(ByVal userId As String, ByVal username As String, ByVal storeCode As String, ByVal storeNameDisplay As String)
        Me.CashierId = userId
        Me.CashierName = username
        Me.StoreId = storeCode
        Me.StoreName = storeCode & "-" & storeNameDisplay
        Me.LoggedInUserId = userId
        Me.StoreNameOnly = storeNameDisplay


        ' ⭐ ADD THESE TWO LINES ⭐
        lblCasheri.Text = Me.CashierName
        lblStroe.Text = Me.StoreName
        lblStoreNameOnly.Text = Me.StoreNameOnly
    End Sub
    Private Sub ApplyButtonPermissions()
        ' Enables the sidebar buttons if the user has at least 'View' permission
        ' If UserSession.IsAdmin is True, this always returns True.
        btnVoide.Enabled = UserSession.HasAction("/pos/btn-void", "view")
        btnDriver.Enabled = UserSession.HasAction("/pos/btn-delivery", "view")
        btnReciptReprint.Enabled = UserSession.HasAction("/pos/btn-reprint", "view")
        btnSetting.Enabled = UserSession.HasAction("/pos/btn-settings", "view")
    End Sub
    ' ---------------------------------------------------------------------
    ' --- 3. Form Load and DataGridView Initialization ---
    ' ---------------------------------------------------------------------
    'Private Async Sub main_pos_system_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    '    FormBorderStyle = FormBorderStyle.None
    '    InitializeDataGridViewLayout()
    '    PaymentConfrim()
    '    ' ⭐ REVISED: Fetch the latest rates from the API instead of hardcoding ⭐
    '    Await FetchExchangeRates()
    '    dg_Items.DefaultCellStyle.WrapMode = DataGridViewTriState.True
    ...
    'End Sub
    Private Async Sub main_pos_system_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ' --- 1. SET FULL SCREEN & HIDE BUTTONS ---
        Me.FormBorderStyle = FormBorderStyle.None
        Me.WindowState = FormWindowState.Normal ' Reset to normal before setting bounds
        Me.Bounds = Screen.PrimaryScreen.Bounds ' Force cover the whole primary monitor

        ' Hide the minimize and maximize buttons specifically
        If btn_Minimize IsNot Nothing Then btn_Minimize.Visible = False
        If btn_Maximize IsNot Nothing Then btn_Maximize.Visible = False
        ...
        ' Initialize Delivery Info
        lblDelivery.Text = "N/A"
        v_DeliveryId = "" ' Note: Changed to empty string to match your v_DeliveryId type
        v_DeliveryDate = Date.Today

        ' Update Store/Cashier labels
        If Me.Controls.ContainsKey("lblStoreName") Then
            Me.Controls("lblStoreName").Text = Me.StoreName
        End If
        ...
        ' --- 3. SHOW LOGIN & PERMISSIONS ---
        frm_login.ShowDialog()
        ApplyButtonPermissions()

        ' --- 4. START SECONDARY DISPLAY ---
        ' This will trigger your logic that finds the second monitor and covers it
        InitializeSecondaryDisplay()
    End Sub
    ' --- frm_pos_sell Code ---
    ...
    '            'v_DisplayForm = New frm_screen_user()

    '            ' Set borderless style and manual positioning first
    '            v_DisplayForm.FormBorderStyle = FormBorderStyle.None
    '            v_DisplayForm.StartPosition = FormStartPosition.CenterScreen ' Crucial for manual positioning
    '            v_DisplayForm.WindowState = FormWindowState.Maximized
    ...
    Public Sub InitializeSecondaryDisplay()
        Try
            If v_DisplayForm IsNot Nothing AndAlso Not v_DisplayForm.IsDisposed Then
                v_DisplayForm.BringToFront()
                Exit Sub
            End If

            v_DisplayForm = New frm_display_second()
            v_DisplayForm.FormBorderStyle = FormBorderStyle.None
            v_DisplayForm.StartPosition = FormStartPosition.Manual
            v_DisplayForm.ShowInTaskbar = False

            Dim screens As Screen() = Screen.AllScreens
            ' Find the screen that is NOT the primary one
            Dim secondary As Screen = screens.FirstOrDefault(Function(s) Not s.Primary)

            If secondary IsNot Nothing Then
                ' FORCE to the secondary screen bounds
                v_DisplayForm.Location = secondary.Bounds.Location
                v_DisplayForm.Size = secondary.Bounds.Size
                v_DisplayForm.WindowState = FormWindowState.Normal ' Use Normal with manual Bounds
                v_DisplayForm.Show()
                ' Re-apply bounds after showing to ensure it covers taskbar
                v_DisplayForm.Bounds = secondary.Bounds
            Else
                ' If no second monitor, show in center of primary for testing
                v_DisplayForm.StartPosition = FormStartPosition.CenterScreen
                v_DisplayForm.Show()
            End If
    ...
    Private Sub InitializeDataGridViewLayout()
        ...
        ' 2. FONT DEFINITIONS (Fixed the comma error)
        Dim font12Bold As New System.Drawing.Font("Microsoft Sans Serif", 13.0F, FontStyle.Bold)
        Dim font12Reg As New System.Drawing.Font("Microsoft Sans Serif", 15.0F, FontStyle.Regular)

        ' 3. APPLY FONTS TO THE GRID
        dg_Items.DefaultCellStyle.Font = font12Reg
        dg_Items.ColumnHeadersDefaultCellStyle.Font = font12Bold
        dg_Items.RowTemplate.Height = 45
        dg_Items.ColumnHeadersHeight = 50

        ' 4. SETUP COLUMNS (Widths increased for Font 12 compatibility)
        ' Format: Name, Header, Width, MinWidth, ReadOnly
        SetupColumn("i_Line", "Line", 45, 40, True)
        SetupColumn("i_ITEMNO", "Item No", 100, 80, True)
        SetupColumn("i_DESC", "Description", 250, 150, True)
        SetupColumn("i_Qty", "Qty", 80, 70, False)
        SetupColumn("i_Price", "Price", 90, 80, True)
        SetupColumn("i_Uom", "UOM", 60, 50, True)
        SetupColumn("i_DisPct", "Disc (%)", 85, 70, False)
        SetupColumn("i_DisAmt", "Disc ($)", 85, 70, False)
        SetupColumn("i_NetAmt", "Amount", 110, 90, True)

        ' --- BUTTON LOGIC (Keep as is, but adjust widths slightly for visibility) ---
        ...
        If Not dg_Items.Columns.Contains("i_Minus") Then
            ...
            With minusCol
                ...
                .Width = 35 ' Increased for Font 12
        ...
        If Not dg_Items.Columns.Contains("i_Remove") Then
            ...
            With removeCol
                ...
                .Text = "X"
                .UseColumnTextForButtonValue = True
                .Width = 40 ' Increased for Font 12
        ...
        Next
    End Sub
    ' --- New Helper Function for Dynamic Multi-Line Descriptions ---
    Private Function FormatDescriptionDynamic(ByVal itemDescription As String) As String
        Const MAX_LINE_LENGTH As Integer = 20 ' Max characters before a forced line break

        If String.IsNullOrEmpty(itemDescription) Then Return ""

        Dim result As New System.Text.StringBuilder()
        Dim words() As String = itemDescription.Trim().Split(New Char() {" "c}, StringSplitOptions.RemoveEmptyEntries)
        ...
        For Each word As String In words
            ...
            Else
                ...
                If currentLineLength + 1 + word.Length > MAX_LINE_LENGTH Then
                    ' Insert line break
                    result.Append(vbCrLf & word)
                    currentLineLength = word.Length
                Else
    ...
    '            ' 2. Make the asynchronous API call and read content
    '            Dim response As HttpResponseMessage = Await httpClient.GetAsync(apiUrl)
    '            Dim responseString As String = Await response.Content.ReadAsStringAsync() ' ⭐ FIX: Read content first

    '            If response.IsSuccessStatusCode Then
    '                ' 3. Parse the JSON response
    '                Dim itemDataList As JArray = JArray.Parse(responseString)
    ...
    '                    Dim v_ITEMNO As String = itemData("itemcode").ToString().Trim()
    '                    Dim itemDescription As String = itemData("itemname").ToString().Trim()
    '                    ' ⭐ MODIFIED: Apply the DYNAMIC formatting here ⭐
    '                    Dim formattedDescription As String = FormatDescriptionDynamic(itemDescription)
    '                    Dim itemUom As String = itemData("uom").ToString().Trim()
    '                    Dim itemPrice As Decimal = 0D
    '                    Decimal.TryParse(itemData("price1").ToString(), itemPrice)
    ...
    '                    ' 4. Check if the item is already in the DataGridView.
    '                    ' ⭐ FIX: Check by itemcode AND uom to allow multiple lines for different packaging.
    '                    For Each GDR As DataGridViewRow In dg_Items.Rows
    '                        If GDR.IsNewRow Then Continue For

    '                        Dim currentItemCode As String = If(GDR.Cells("i_ITEMNO").Value IsNot Nothing, GDR.Cells("i_ITEMNO").Value.ToString().Trim(), "")
    '                        Dim currentUom As String = If(GDR.Cells("i_Uom").Value IsNot Nothing, GDR.Cells("i_Uom").Value.ToString().Trim(), "")

    '                        ' Check if the scanned item (by item code and UOM) matches an existing row
    '                        If currentItemCode = v_ITEMNO AndAlso currentUom = itemUom Then
    '                            ' Item Found: Increase Quantity for this specific UOM line
    '                            Dim currentQty As Decimal
    '                            If Not Decimal.TryParse(GDR.Cells("i_Qty").Value.ToString(), currentQty) Then currentQty = 0D
    ...
    '            ElseIf response.StatusCode = Net.HttpStatusCode.NotFound Then ' ⭐ FIX: Handle 404 explicitly
    '                MessageBox.Show("Barcode [" & barcodeValue & "] not found in the system or API endpoint missing.", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Warning)
    '            Else
    '                ' Handle other HTTP error codes
    '                Dim errorDetails = responseString ' ⭐ FIX: Use the previously read content
    '                MessageBox.Show($"API Error: {response.StatusCode}. Details: {errorDetails}", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
    '            End If
    ...
    '    End If
    'End Sub
    ' NOTE: The old txt_Barcode_KeyDown event logic is moved into ProcessSelectedBarcode.
    Private Async Sub txt_Barcode_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_Barcode.KeyDown
        If e.KeyCode = Keys.Enter Then
            Dim barcodeValue As String = txt_Barcode.Text.Trim()
            If String.IsNullOrEmpty(barcodeValue) Then Exit Sub
            ...
            Await ProcessSelectedBarcode(barcodeValue)

            ' The cleanup and focus logic is inside ProcessSelectedBarcode
        End If
    End Sub

    Private Async Sub btnItemCode_Click(sender As Object, e As EventArgs) Handles btnItemCode.Click
        Try
            ' 1. Create the search form instance
            Dim findItemForm As New frm_find_item
            findItemForm.FormBorderStyle = FormBorderStyle.None

            ' 2. Show the dialog and check the result (DialogResult.OK means 'Select' or double-click was used)
            If findItemForm.ShowDialog() = DialogResult.OK Then
                ' 3. Directly read the selected value from the form's public property
                Dim selectedItemCodeOrBarcode As String = findItemForm.F_Item

                If Not String.IsNullOrEmpty(selectedItemCodeOrBarcode) Then
                    ' 4. Call the centralized processing function
                    ' We skip putting the value in txt_Barcode and call the processor directly
                    Await ProcessSelectedBarcode(selectedItemCodeOrBarcode)
                End If
            End If

        Catch ex As Exception
            MessageBox.Show("Error selecting item: " & ex.Message, V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub

    '' <summary>
    '' Processes a selected item code or barcode by calling the API and adding the item to the grid.
    '' This is called from both direct scan (KeyDown) and the item lookup form (btnItemCode_Click).
    '' </summary>
    '' <param name="barcodeValue">The item code or barcode to look up.</param>
    'Private Async Function ProcessSelectedBarcode(ByVal barcodeValue As String) As Task
    '    If String.IsNullOrEmpty(barcodeValue) Then Exit Function

    '    Try
    '        ' 1. Construct the API URL (use /api/scanbarcode for direct lookup)
    '        ' NOTE: This uses the direct barcode lookup which is suitable for a specific item code or barcode.
    '        Dim apiUrl As String = $"{API_BASE_URL}/api/scanbarcode?barcode={Uri.EscapeDataString(barcodeValue)}"
    ...
    '                If Not itemFound Then
    '                    Dim newQty As Decimal = 1D
    '                    Dim netAmount As Decimal = newQty * itemPrice
    ...
    '                End If
    '            Else
    '                MessageBox.Show("Selected item [" & barcodeValue & "] not found in the system.", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Warning)
    '            End If

    '        ElseIf response.StatusCode = Net.HttpStatusCode.NotFound Then
    '            MessageBox.Show("Selected item [" & barcodeValue & "] not found in the system or API endpoint missing.", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Warning)
    '        Else
    '            Dim errorDetails = responseString
    '            MessageBox.Show($"API Error: {response.StatusCode}. Details: {errorDetails}", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
    '        End If
    ...
    '        MessageBox.Show("Error processing API response: " & ex.Message, V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
    '    Finally
    '        ' Always clear the textbox and set focus for the next scan/entry
    '        txt_Barcode.Clear()
    '        txt_Barcode.Focus()
    '    End Try
    'End Function
    ...
    '    Try
    '        ' 1. API call (Now searches Barcode OR ItemCode thanks to the API fix above)
    '        Dim apiUrl As String = $"{API_BASE_URL}/api/scanbarcode?barcode={Uri.EscapeDataString(barcodeValue)}"
    '        Dim response As HttpResponseMessage = Await httpClient.GetAsync(apiUrl)
    '        Dim responseString As String = Await response.Content.ReadAsStringAsync()
    ...
    '            If itemDataList IsNot Nothing AndAlso itemDataList.Count > 0 Then
    '                Dim itemData As JObject = itemDataList(0)

    '                ' ⭐ ALWAYS match using "itemcode" from the database, not the input value
    '                ' This ensures that if you scan a barcode, it finds the parent Item Code.
    '                Dim dbItemCode As String = itemData("itemcode").ToString().Trim()
    '                Dim itemName As String = itemData("itemname").ToString().Trim()
    '                Dim itemUom As String = If(itemData("uom") IsNot Nothing, itemData("uom").ToString().Trim(), "PCS")
    '                Dim itemPrice As Decimal = 0D
    '                Decimal.TryParse(itemData("price1")?.ToString(), itemPrice)
    '                Dim imgFileName As String = If(itemData("item_image") IsNot Nothing, itemData("item_image").ToString(), "")
    '                Dim fullImageUrl As String = ""
    '                If Not String.IsNullOrEmpty(imgFileName) Then
    '                    ' Construct the full URL pointing to your API's static images folder
    '                    fullImageUrl = $"{API_BASE_URL}/images/{imgFileName}"
    '                End If
    '                Dim itemFound As Boolean = False

    '                ' 2. Check if this ItemCode + UOM is already in the DataGridView
    '                For Each row As DataGridViewRow In dg_Items.Rows
    '                    If row.IsNewRow Then Continue For

    '                    Dim gridCode As String = If(row.Cells("i_ITEMNO").Value IsNot Nothing, row.Cells("i_ITEMNO").Value.ToString().Trim(), "")
    '                    Dim gridUom As String = If(row.Cells("i_Uom").Value IsNot Nothing, row.Cells("i_Uom").Value.ToString().Trim(), "")

    '                    ' Match by standardized ItemCode from DB
    '                    If gridCode = dbItemCode AndAlso gridUom = itemUom Then
    '                        Dim currentQty As Decimal = CDec(row.Cells("i_Qty").Value)
    '                        row.Cells("i_Qty").Value = (currentQty + 1).ToString(v_FormatNo)
    '                        UpdateRowTotals(row)
    '                        itemFound = True
    '                        Exit For
    '                    End If
    ...
    '                If Not itemFound Then
    '                    ' 3. Add new row if not in list
    '                    Me.dg_Items.Rows.Add(dg_Items.Rows.Count + 1, dbItemCode, FormatDescriptionDynamic(itemName), "-", "1.00", "+", itemPrice.ToString(v_FormatNo), itemUom, "0.00", "0.00", itemPrice.ToString(v_FormatNo), "X")
    '                    UpdateTotalSummary()
    '                End If

    '                UpdateSecondaryDisplay()
    '            Else
    '                MessageBox.Show("Code [" & barcodeValue & "] not found in Database.", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Warning)
    '            End If
    '        End If
    '    Catch ex As Exception
        ...
        Try
            ...
            If response.IsSuccessStatusCode Then
                ...
                If itemDataList IsNot Nothing AndAlso itemDataList.Count > 0 Then
                    ...
                    ' ⭐ POINT OF UPDATE: Capture 'item_image' from API response
                    Dim imgFileName As String = If(itemData("item_image") IsNot Nothing, itemData("item_image").ToString(), "")
                    Dim fullImageUrl As String = ""

                    If Not String.IsNullOrEmpty(imgFileName) Then
                        ' Assuming your server serves images at http://localhost:5005/images/
                        fullImageUrl = $"{API_BASE_URL}/images/{imgFileName}"
                    End If
                    ...
                    For Each row As DataGridViewRow In dg_Items.Rows
                        If row.IsNewRow Then Continue For
                        If row.Cells("i_ITEMNO").Value.ToString().Trim() = dbItemCode AndAlso row.Cells("i_Uom").Value.ToString().Trim() = itemUom Then
                            Dim currentQty As Decimal = CDec(row.Cells("i_Qty").Value)
                            row.Cells("i_Qty").Value = (currentQty + 1).ToString(v_FormatNo)
                            UpdateRowTotals(row)
                            itemFound = True
                    ...
                    ' ⭐ POINT OF UPDATE: Pass the constructed URL to update the display
                    UpdateSecondaryDisplay(fullImageUrl)
                Else
                    MessageBox.Show("Code [" & barcodeValue & "] not found in Database.", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Warning)
                End If
            End If
    ...
    Private Sub dg_Items_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles dg_Items.CellEndEdit
        ...
        Select Case ColName
            ...
            Case "i_DisPct"
                ...
                ' Safely handles Nothing/DBNull by treating it as an empty string ("")
                If Not Decimal.TryParse(CType(row.Cells("i_DisPct").Value, String), newDisPct) Then newDisPct = 0D
            ...
            Case "i_DisAmt"
                ...
                ' Safely handles Nothing/DBNull by treating it as an empty string ("")
                If Not Decimal.TryParse(CType(row.Cells("i_DisAmt").Value, String), newDisAmt) Then newDisAmt = 0D
    ...
    '        ' 1. Read the Totals from the main form
    '        Dim subTotal_Net As Decimal = 0D
    '        Dim checkDiscountAmt As Decimal = 0D
    '        Dim netTotalDue As Decimal = 0D

    '        Decimal.TryParse(txtSubTotal.Text, subTotal_Net)         ' SubTotal (Net)
    '        Decimal.TryParse(txtCheckDiscountAmt.Text, checkDiscountAmt) ' Check Discount Amount
    '        Decimal.TryParse(txtNetTotal.Text, netTotalDue)          ' GrandTotal (Net Total Due)
    ...
    '        ' ⭐ MODIFIED: Pass SubTotal, Discount, and GrandTotal to the secondary display.
    '        v_DisplayForm.UpdateDisplayItems(currentItems, netTotalDue, subTotal_Net, checkDiscountAmt)
    '    End If
    'End Sub
    ' ⭐ REVISED: Added lastItemImageUrl parameter to pass to the display form
    Private Sub UpdateSecondaryDisplay(Optional ByVal lastItemImageUrl As String = "")
        If v_DisplayForm IsNot Nothing AndAlso Not v_DisplayForm.IsDisposed Then
            Dim currentItems As List(Of DisplayItemInfo) = GetCurrentItemsForDisplay()
            ...
            Decimal.TryParse(txtSubTotal.Text, subTotal_Net)
            Decimal.TryParse(txtCheckDiscountAmt.Text, checkDiscountAmt)
            Decimal.TryParse(txtNetTotal.Text, netTotalDue)

            Dim totalyqty As Decimal = 0D
            Decimal.TryParse(txtQuantity.Text, totalyqty)

            ' 2. Read KHR and THB totals from labels
            Dim khrTotal As String = lblAmtKHR.Text
            Dim thbTotal As String = lblAmtTHB.Text

            ' 3. Attach currency totals to the first item for easy passing
            If currentItems.Count > 0 Then
                currentItems(0).TotalKHR = khrTotal
                currentItems(0).TotalTHB = thbTotal
                currentItems(0).TotalPaid = netTotalDue.ToString("N2")
            ...
            ' 4. Call the display form method and PASS THE IMAGE URL
            v_DisplayForm.UpdateDisplayItems(currentItems, netTotalDue, subTotal_Net, checkDiscountAmt, lastItemImageUrl)
        End If
    End Sub
    ...
    ' ---------------------------------------------------------------------
    Private Sub btnClear_Click(sender As Object, e As EventArgs) Handles btnClear.Click
        ' Determine if the call is manual (sender is the button) or automatic (sender is Nothing)
        Dim isManualClear As Boolean = (sender IsNot Nothing)
        ...
        ' Disable message for clear

        'If isManualClear Then
        '    ' Ask for confirmation only if the button was manually clicked
    ...
    ' REMOVE THE EXISTING PrintReceiptExecute (which uses PrintPreviewDialog1)
    ' AND REPLACE IT WITH THIS NEW FUNCTION:
    Private Sub PrintReceiptAuto(ByVal receiptNo As String)
        ' 1. Generate Content
        v_ReceiptContentToPrint = GenerateReceiptContent(receiptNo)

        Try
            ' 2. Assign the printer name and check for validity
            PrintDocument1.PrinterSettings.PrinterName = POS_PRINTER_NAME

            If Not PrintDocument1.PrinterSettings.IsValid Then
                MessageBox.Show("POS Printer '" & POS_PRINTER_NAME & "' is not found or is invalid. Cannot print receipt automatically.", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
                Exit Sub
            End If

            ' 3. Execute automatic print
            PrintDocument1.Print() ' ⭐ THIS IS THE AUTO-PRINT CALL ⭐

        Catch ex As InvalidOperationException
            MessageBox.Show("Printing Error: Check printer connections and settings. Details: " & ex.Message, V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
        Catch ex As Exception
            MessageBox.Show("Error during automatic receipt printing: " & ex.Message, V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    '        MessageBox.Show("Cannot complete transaction: No items added.", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Warning)
    '        Exit Sub
    '    End If
    ...
    '    ' ⭐ FLAG: Check if any of the collected payments is cash ⭐
    '    Dim isCashPaymentMade As Boolean = False
    ...
    '        ' Check if this payment is cash (we assume USD, KHR, THB cash payments start with "Cash")
    '        If payMethodText.StartsWith("Cash") Then
    '            isCashPaymentMade = True
    '        End If
    ...
    '            'MessageBox.Show($"Transaction {finalReceiptNo} posted successfully via API! ✅", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Information)
    '            MessageBox.Show($"SETTLEMENT SUCCESS! Have a great day! 🎉", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Information)

    '            ' ⭐ STEP 6: CONDITIONAL CASH DRAWER OPEN ⭐
    '            If isCashPaymentMade Then
    '                If Not PosPrinterCommands.OpenCashDrawer(POS_PRINTER_NAME) Then
    '                    MessageBox.Show("Transaction was successful, but failed to open cash drawer.", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Warning)
    '                End If
    '            End If

    '            ' ⭐ ACTION: Auto Print the receipt instead of previewing
    '            PrintReceiptAuto(finalReceiptNo)

    '            ' ⭐ ACTION: Clear the UI for the next transaction immediately after printing
    '            btnClear_Click(Nothing, EventArgs.Empty)
    '        Else
    '            ' Handle API errors (400, 500, etc.)
    '            Dim errorJson As JObject = JObject.Parse(responseString)
    ...
    Private Async Sub btnPay_Click(sender As Object, e As EventArgs) Handles btnPay.Click
        ...
        ' ⭐ Delivery Data Gathering
        Dim deliveryName As String = lblDelivery.Text
        Dim deliveryId As String = v_DeliveryId
        ' ⭐ FIX: Use String comparison, NOT mathematical comparison ( > 0 )
        Dim isDeliverySale As Boolean = Not String.IsNullOrEmpty(deliveryId) AndAlso deliveryId <> "0"
        ' --- 2. PREPARE RECEIPT DATE ---
        ...
        ' --- 4. BUILD SALES JSON PAYLOAD ---
        ' ⭐ CORRECTED: Removed local receiptNoPrefix. API handles numbering.
        Dim headerObject As New JObject From {
          {"receipt_no", ""},
          {"store_id", Me.StoreId},
          {"store_name", Me.StoreNameOnly},
          {"receipt_date", receiptDate.ToString("yyyy-MM-ddTHH:mm:ss")},
          {"receipt_batchinvoice", ""},
          {"receipt_status", 1},
          {"cashier_id", Me.CashierId},
          {"cashier_name", Me.CashierName},
          {"coment", ""},
          {"sub_total", totalGrossAmt},
          {"line_discount", totalLineDiscount},
          {"global_discount", checkDisAmt},
          {"global_discount_pct", checkDisPct},
          {"ship_fee", shippingFee},
          {"total_discount", totalDiscount},
          {"total_discount_vat", vatAmt},
        ...
        ' (Details and Payments logic stays the same as your snippet)
        Dim detailsArray As New JArray()
        For Each row As DataGridViewRow In dg_Items.Rows
            If row.IsNewRow Then Continue For
            Dim itemQty As Decimal = CDec(row.Cells("i_Qty").Value)
        ...
        For Each pmtRow As DataGridViewRow In dg_Payment.Rows
            If pmtRow.IsNewRow Then Continue For
            Dim payMethodText As String = pmtRow.Cells("PAY_METHOD").Value.ToString()
            If payMethodText.StartsWith("Cash") Then isCashPaymentMade = True
            Dim bankCodeText As String = If(pmtRow.Cells("BANK").Value IsNot Nothing, pmtRow.Cells("BANK").Value.ToString(), Nothing)
            ' ... (Currency rate logic) ...
            Dim paymentDetailObject As New JObject From {
              {"receipt_line", pmtRow.Index + 1},
              {"pay_method", JToken.FromObject(payMethodText)},
              {"cur_code", pmtRow.Cells("CURRENCY").Value.ToString()},
              {"cur_rate", 1D}, ' Simplified for example
              {"rec_amt", CDec(pmtRow.Cells("REC_AMT").Value)},
              {"bank_code", If(String.IsNullOrEmpty(bankCodeText), JValue.CreateNull(), JToken.FromObject(bankCodeText))},
              {"change_amt", 0D}
          }
            paymentDetailsArray.Add(paymentDetailObject)
        Next
        ...
        Dim requestBody As New JObject From {
          {"header", headerObject},
          {"details", detailsArray},
          {"payments", paymentDetailsArray}
      }

        ' --- 5. EXECUTE ASYNCHRONOUS API POST ---
        Try
            Dim content As New StringContent(requestBody.ToString(), System.Text.Encoding.UTF8, "application/json")
            Dim response As HttpResponseMessage = Await httpClient.PostAsync($"{API_BASE_URL}/api/pos/sales", content)
            Dim responseString As String = Await response.Content.ReadAsStringAsync()

            If response.IsSuccessStatusCode Then
                Dim successJson As JObject = JObject.Parse(responseString)

                ' ⭐ CAPTURE THE DYNAMIC NUMBER FROM SERVER (INV001 or IN001)
                Dim finalReceiptNo As String = successJson("receipt_no").ToString()

                ' 1. Clean the string (Remove "USD" and any extra spaces)
                Dim cleanChangeText As String = txtChange.Text.Replace("USD", "").Trim()

                ' 2. Declare variables to hold the numeric results
                Dim changeValue As Decimal = 0D
                Dim khrRate As Decimal = 0D
                Dim thbRate As Decimal = 0D

                ' 3. Parse EVERYTHING before calculating
                Decimal.TryParse(cleanChangeText, changeValue)
                Decimal.TryParse(txtKHRRate.Text, khrRate)
                Decimal.TryParse(txtTHBRate.Text, thbRate)

                ' 4. Apply sensible defaults if rates are missing
                If khrRate <= 0D Then khrRate = 4100D
                If thbRate <= 0D Then thbRate = 30D

                ' 5. ⭐ CRITICAL FIX: Use the parsed numeric variables (changeValue, khrRate) 
                '    DO NOT use the TextBoxes directly in multiplication.
                frm_ChangeAmt.lbl_ChangeUSD.Text = changeValue.ToString("N2") & " USD"
                frm_ChangeAmt.lbl_ChangeKHR.Text = (changeValue * khrRate).ToString("N0") & " KHR"
                frm_ChangeAmt.lbl_ChangeBAT.Text = (changeValue * thbRate).ToString("N0") & " THB"

                frm_ChangeAmt.ShowDialog()
                '  MessageBox.Show($"SETTLEMENT SUCCESS! Receipt No: {finalReceiptNo}", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Information)

                If isCashPaymentMade Then
                    PosPrinterCommands.OpenCashDrawer(POS_PRINTER_NAME)
                End If

                ' ⭐ Print using the Server's assigned Number
                PrintReceiptAuto(finalReceiptNo)

                ' Clear for next sale
                btnClear_Click(Nothing, EventArgs.Empty)
            Else
                MessageBox.Show("API Error: " & responseString, V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
        Catch ex As Exception
            MessageBox.Show("System Error: " & ex.Message, V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    '        Dim errorFont As New System.Drawing.Font("Arial", 10)
    '        e.Graphics.DrawString("[LOGO MISSING/ERROR]", errorFont, System.Drawing.Brushes.Red, leftMargin, yPosition)
    '        yPosition += errorFont.GetHeight(e.Graphics) * 1.5F
    '    End Try
    ...
    '    ' Text Content Setup
    '    Dim font As New System.Drawing.Font("Courier New", 8, System.Drawing.FontStyle.Regular)
    ...
    '    ' --- 1. Margin & Padding Configuration ---
    '    Dim topPadding As Single = 9.0F    ' Space above the logo
    '    Dim bottomPadding As Single = 12.0F ' Space between logo and first text line
    '    Dim leftMargin As Single = 1.0F     ' Slight nudge from the physical left edge

    '    ' Path to your logo
    '    Dim logoPath As String = Application.StartupPath & "\" & "CompanyLogo.png"

    '    ' Start Y position at top padding
    '    Dim yPosition As Single = topPadding

    '    ' --- 2. Draw Logo with Improved Padding ---
    '    Try
    '        If System.IO.File.Exists(logoPath) Then
    '            Using logoImage As Image = Image.FromFile(logoPath)
    '                ' Define the logo width (180 is usually safe for 80mm/3-inch paper)
    '                Dim targetWidth As Integer = 180

    '                ' Maintain Aspect Ratio
    '                Dim ratio As Double = logoImage.Height / logoImage.Width
    '                Dim targetHeight As Integer = CInt(targetWidth * ratio)

    '                ' Center the logo (Assuming 80mm paper is approx 280-300 pixels wide in 100dpi)
    '                ' Adjust printAreaWidth if the logo looks off-center
    '                Dim printAreaWidth As Single = 280.0F
    '                Dim xPosition As Single = (printAreaWidth - targetWidth) / 2.0F

    '                ' Draw the image
    '                e.Graphics.DrawImage(logoImage, xPosition, yPosition, targetWidth, targetHeight)

    '                ' Update Y: Add the image height PLUS the bottom padding
    '                yPosition += targetHeight + bottomPadding
    '            End Using
    '        Else
    '            ' If file is missing, start text after a small gap
    '            yPosition += 10.0F
    '        End If
    '    Catch ex As Exception
    '        ' Optional: log error but keep printing text
    '        yPosition += 10.0F
    '    End Try

    '    ' --- 3. Draw Receipt Text Content ---
    '    Dim font As New System.Drawing.Font("Courier New", 8, System.Drawing.FontStyle.Regular)
    '    Dim brush As New System.Drawing.SolidBrush(System.Drawing.Color.Black)
    '    Dim lineHeight As Single = font.GetHeight(e.Graphics) * 1.1F

    '    ' Ensure content isn't empty
    '    If Not String.IsNullOrEmpty(v_ReceiptContentToPrint) Then
    '        Dim lines() As String = v_ReceiptContentToPrint.Split(New String() {vbCrLf}, StringSplitOptions.None)
    ...
    '    receiptLines.AppendLine(CenterLine(Me.StoreNameOnly))
    ...
    '    If Not String.IsNullOrEmpty(v_DeliveryId) AndAlso v_DeliveryId <> "0" Then
    ...
    '    receiptLines.AppendLine(String.Format("{0,-25} {1,15}", "**GRAND TOTAL (USD):**", grandTotal.ToString("N2")))
    ...
    '    Dim receiptLines As New System.Text.StringBuilder()
    '    Const TOTAL_WIDTH As Integer = 40
    '    Const DASH_LINE As String = "----------------------------------------"
    '    Const EQUAL_LINE As String = "========================================"

    '    ' Helper for centering
    '    Dim CenterLine = Function(t As String) As String
    '                         Dim s = If(t, "")
    '                         Return If(s.Length >= TOTAL_WIDTH, s.Substring(0, TOTAL_WIDTH), New String(" "c, (TOTAL_WIDTH - s.Length) \ 2) & s)
    '                     End Function

    '    ' Header
    '    ' --- ADDED: PRE-CHECK INDICATOR ---
    '    If receiptNo = "CHECK-PRINT" Then
    '        receiptLines.AppendLine(CenterLine("*** PRE-CHECK / UNPAID ***"))
    '        receiptLines.AppendLine(CenterLine("This is not a final receipt"))
    '        receiptLines.AppendLine(DASH_LINE)
    '    End If

    '    receiptLines.AppendLine(CenterLine(Me.StoreNameOnly))
    '    receiptLines.AppendLine(CenterLine("PHNOM PENH, CAMBODIA"))
    '    receiptLines.AppendLine(CenterLine("Tel: 020-386-XXXX"))
    '    receiptLines.AppendLine(DASH_LINE)

    '    receiptLines.AppendLine(String.Format("{0,-10}{1,30}", "Invoice:", receiptNo))
    '    receiptLines.AppendLine(String.Format("{0,-10}{1,30}", "Date:", DateTime.Now.ToString("dd/MM/yyyy HH:mm")))
    '    receiptLines.AppendLine(EQUAL_LINE)

    '    ' Item Columns: Qty(4) | Item(16) | Price(9) | Total(11)
    '    receiptLines.AppendLine(String.Format("{0,-4}{1,-16}{2,9}{3,11}", "QTY", "ITEMS", "PRICE", "AMOUNT"))
    '    receiptLines.AppendLine(DASH_LINE)

    '    For Each row As DataGridViewRow In dg_Items.Rows
    '        If row.IsNewRow Then Continue For

    '        Dim qty As String = CDec(row.Cells("i_Qty").Value).ToString("0")
    '        Dim desc As String = row.Cells("i_DESC").Value.ToString().Trim()
    '        Dim price As Decimal = CDec(row.Cells("i_Price").Value)
    '        Dim total As Decimal = CDec(row.Cells("i_NetAmt").Value)

    '        ' Main Line
    '        Dim firstDesc = If(desc.Length > 16, desc.Substring(0, 16), desc)
    '        receiptLines.AppendLine(String.Format("{0,-4}{1,-16}{2,9:N2}{3,11:N2}", qty, firstDesc, price, total))

    '        ' Wrap long description
    '        If desc.Length > 16 Then
    '            Dim remaining = desc.Substring(16).Trim()
    '            While remaining.Length > 0
    '                Dim chunk = If(remaining.Length > 30, remaining.Substring(0, 30), remaining)
    '                receiptLines.AppendLine(String.Format("    {0}", chunk))
    '                remaining = If(remaining.Length > 30, remaining.Substring(30).Trim(), "")
    '            End While
    '        End If
    '    Next
    ...
    '    ' Totals
    '    Dim grandTotal As Decimal : Decimal.TryParse(txtNetTotal.Text, grandTotal)
    '    receiptLines.AppendLine(String.Format("{0,-20}{1,20:N2}", "**GRAND TOTAL (USD)**", grandTotal))
    '    receiptLines.AppendLine(EQUAL_LINE)

    '    ' Currency & Change
    '    Dim paid As Decimal : Decimal.TryParse(txtPaid.Text, paid)
    '    Dim rate As Decimal = 4100 ' Example rate
    '    Dim changeUSD As Decimal = Math.Max(0, paid - grandTotal)

    '    receiptLines.AppendLine(String.Format("{0,-20}{1,20:N2}", "Cash Received $", paid))
    '    receiptLines.AppendLine(String.Format("{0,-20}{1,20:N2}", "Change USD $", changeUSD))
    '    receiptLines.AppendLine(String.Format("{0,-20}{1,20:N0}", "Change KHR Riels", changeUSD * rate))

    '    ' Footer Spacing
    '    receiptLines.AppendLine(DASH_LINE)
    '    receiptLines.AppendLine(CenterLine("THANK YOU FOR YOUR PURCHASE!"))
    '    receiptLines.AppendLine(CenterLine("Please come again."))
    '    receiptLines.AppendLine(CenterLine(DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss")))
    '    receiptLines.AppendLine()
    '    receiptLines.AppendLine()
    '    receiptLines.AppendLine()
    '    receiptLines.AppendLine(".")

    '    ' Feed lines for the cutter
    ...
    '    ' Helper for centering text based on 40 character width
    '    Dim CenterLine = Function(t As String) As String
    '                         Dim s = If(t, "")
    '                         Return If(s.Length >= TOTAL_WIDTH, s.Substring(0, TOTAL_WIDTH), New String(" "c, (TOTAL_WIDTH - s.Length) \ 2) & s)
    '                     End Function
    ...
    '    ' Header labels updated to match template image: Check/Date/Staff
    '    If receiptNo = "CHECK-PRINT" Then
    '        receiptLines.AppendLine(String.Format("{0,-10}{1,30}", "Check:", "Pre-Print"))
    '    Else
    '        receiptLines.AppendLine(String.Format("{0,-10}{1,30}", "Invoice:", receiptNo))
    '    End If

    '    receiptLines.AppendLine(String.Format("{0,-10}{1,30}", "Date:", DateTime.Now.ToString("dd/MM/yyyy HH:mm")))
    '    receiptLines.AppendLine(String.Format("{0,-10}{1,30}", "Staff:", Me.CashierName))
    '    receiptLines.AppendLine(EQUAL_LINE)

    '    ' --- 2. Item Columns Section ---
    '    ' Format: Qty(4) | Item(16) | Price(9) | Total(11)
    '    receiptLines.AppendLine(String.Format("{0,-4}{1,-16}{2,9}{3,11}", "QTY", "ITEMS", "PRICE", "AMOUNT"))
    '    receiptLines.AppendLine(DASH_LINE)
    ...
    '        ' Main Line: Fits description into 16 chars
    '        Dim firstDesc = If(desc.Length > 16, desc.Substring(0, 16), desc)
    '        receiptLines.AppendLine(String.Format("{0,-4}{1,-16}{2,9:N2}{3,11:N2}", qty, firstDesc, price, total))

    '        ' Wrap long description if it exceeds 16 chars
    '        If desc.Length > 16 Then
    '            Dim remaining = desc.Substring(16).Trim()
    '            While remaining.Length > 0
    '                Dim chunk = If(remaining.Length > 30, remaining.Substring(0, 30), remaining)
    ...
    '    Dim subTotalNet As Decimal : Decimal.TryParse(txtSubTotal.Text, subTotalNet)
    '    Dim checkDisAmt As Decimal : Decimal.TryParse(txtCheckDiscountAmt.Text, checkDisAmt)
    '    Dim grandTotal As Decimal : Decimal.TryParse(txtNetTotal.Text, grandTotal)

    '    ' Labels updated to match template: Sub Total / Discount / Total
    '    receiptLines.AppendLine(String.Format("{0,-20}{1,20:N2}", "Sub Total", subTotalNet))
    '    receiptLines.AppendLine(String.Format("{0,-20}{1,20:N2}", "Discount", checkDisAmt))
    '    receiptLines.AppendLine(String.Format("{0,-20}{1,20:N2}", "Total", grandTotal))
    '    receiptLines.AppendLine(EQUAL_LINE)

    '    ' --- 4. Guest & Signature Section (Conditional for Check Print) ---
    '    If receiptNo = "CHECK-PRINT" Then
    '        receiptLines.AppendLine()
    '        receiptLines.AppendLine("Guest name:")
    '        receiptLines.AppendLine("                ------------------------")
    '        receiptLines.AppendLine()
    '        receiptLines.AppendLine("Signature:")
    '        receiptLines.AppendLine("                ------------------------")
    '        receiptLines.AppendLine()
    '    Else
    ...
    '    ' --- 5. Footer Section ---
    '    receiptLines.AppendLine(CenterLine("Thanks You!!!"))
    '    receiptLines.AppendLine(CenterLine("Have a nice day!"))
    '    ' Phone and Web from template image
    '    receiptLines.AppendLine(CenterLine("Tel: (855)23 884 242, 017 222 009"))
    '    receiptLines.AppendLine(CenterLine("www.positronmultiverse.com.kh"))

    '    ' Feed lines for paper cutter
    '    receiptLines.AppendLine(vbCrLf & vbCrLf & vbCrLf & vbCrLf & vbCrLf)
    '    receiptLines.AppendLine(".")
    ...
    Private Sub PrintDocument1_PrintPage(sender As Object, e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ' --- Font Configuration ---
        ' Courier New is the standard for receipts because it aligns perfectly
        Dim printFont As New System.Drawing.Font("Courier New", 8.0F, System.Drawing.FontStyle.Regular)
        Dim boldFont As New System.Drawing.Font("Courier New", 8.0F, System.Drawing.FontStyle.Bold)
        Dim brush As New System.Drawing.SolidBrush(System.Drawing.Color.Black)

        ' Calculate layout
        Dim paperWidth As Single = e.PageSettings.PrintableArea.Width ' ~283 points for 80mm
        Dim lineHeight As Single = printFont.GetHeight(e.Graphics)

        ' Measure 40 chars to center the block
        Dim sampleWidth As Single = e.Graphics.MeasureString("0000000000000000000000000000000000000000", printFont).Width
        Dim leftMargin As Single = (paperWidth - sampleWidth) / 2
        If leftMargin < 0 Then leftMargin = 5 ' Minimum 5pt margin

        Dim yPosition As Single = 10.0F

        ' --- 1. Logo (Centered) ---
        Try
            Dim logoPath As String = Application.StartupPath & "\CompanyLogo.png"
            If System.IO.File.Exists(logoPath) Then
                Using logoImg As Image = Image.FromFile(logoPath)
                    Dim targetWidth As Integer = 160
                    Dim targetHeight As Integer = CInt((logoImg.Height / logoImg.Width) * targetWidth)
                    e.Graphics.DrawImage(logoImg, leftMargin + (sampleWidth - targetWidth) / 2, yPosition, targetWidth, targetHeight)
                    yPosition += targetHeight + 10
                End Using
            End If
        Catch : yPosition += 5 : End Try

        ' --- 2. Print Content ---
        Dim lines() As String = v_ReceiptContentToPrint.Split(New String() {vbCrLf}, StringSplitOptions.None)

        For Each line As String In lines
            ' Highlight Grand Total or Headers with Bold if they contain specific markers
            Dim currentFont As System.Drawing.Font = printFont
            If line.Contains("**") Or line.Contains("==") Then
                currentFont = boldFont
            End If

            e.Graphics.DrawString(line.Replace("**", ""), currentFont, brush, leftMargin, yPosition)
            yPosition += lineHeight
        Next

        ' --- 3. Footer Spacer (The "Feed") ---
        ' This pushes the text past the printer's cutter blade
        yPosition += (lineHeight * 6)

        e.HasMorePages = False
    End Sub
    Private Sub btnSell_Click(sender As Object, e As EventArgs) Handles btnSell.Click
        Dim url As String = "http://172.29.29.56:8889/"
        ...
        Catch ex As Exception
            MessageBox.Show("Unable to open browser." & vbCrLf & ex.Message,
                            "Error",
    ...
    Private Function GenerateReceiptContent(ByVal receiptNo As String) As String
        ...
        ' Helper for dynamic decimal formatting (5.00 -> 5, 5.12 -> 5.12)
        Dim FormatPrice = Function(num As Decimal) As String
        ...
        If receiptNo = "CHECK-PRINT" Then
            receiptLines.AppendLine(String.Format("Check: {0}", "Guest Check"))
        Else
            receiptLines.AppendLine(String.Format("Check: {0}", receiptNo))
        End If

        receiptLines.AppendLine(String.Format("Date : {0}", DateTime.Now.ToString("dd-MM-yyyy HH:mm")))
        receiptLines.AppendLine(String.Format("Staff: {0}", Me.CashierName))
        receiptLines.AppendLine(EQUAL_LINE)

        For Each row As DataGridViewRow In dg_Items.Rows
            If row.IsNewRow Then Continue For

            ' 1. Clean description: Merges multiline UI text into one single string
            Dim rawDesc As String = row.Cells("i_DESC").Value.ToString().Replace(vbCrLf, " ").Replace(vbLf, " ").Trim()
            ...
            ' 2. Split description into chunks of 20 characters
            Dim chunks As New List(Of String)
            For i As Integer = 0 To fullDesc.Length - 1 Step 20
                chunks.Add(If(i + 20 <= fullDesc.Length, fullDesc.Substring(i, 20), fullDesc.Substring(i)))
            Next

            ' 3. Print the First Line
            ' Layout: Qty(3) + Space(1) + NamePart(20) + Space(1) + Price/Space(7) + Space(1) + Total(7) = 40
            Dim firstChunk As String = If(chunks.Count > 0, chunks(0), "")

            ' Logic: If Qty is 1, don't show the unit price (show blank instead)
            ...
            ' 4. Print Remaining Chunks (Wrapped text indented by 4 spaces)
        ...
        ' --- 3. Summary Totals Section ---
        Dim totalQty As Decimal : Decimal.TryParse(txtQuantity.Text, totalQty)
        Dim subTotalNet As Decimal : Decimal.TryParse(txtSubTotal.Text, subTotalNet)
        Dim checkDisAmt As Decimal : Decimal.TryParse(txtCheckDiscountAmt.Text, checkDisAmt)
        Dim grandTotal As Decimal : Decimal.TryParse(txtNetTotal.Text, grandTotal)


        receiptLines.AppendLine(String.Format("{0,-30} {1,9:N2}", "Sub Total", subTotalNet))
        receiptLines.AppendLine(String.Format("{0,-30} {1,9:N2}", "Discount", checkDisAmt))

        ' --- 4. Format Selection (Guest Check vs Close Check) ---
        If receiptNo = "CHECK-PRINT" Then
            ' **Total** markers used for bolding in your PrintPage event
            receiptLines.AppendLine(String.Format("{0,-30} {1,13:N2}", "**Total**", grandTotal))
            receiptLines.AppendLine(EQUAL_LINE)
            receiptLines.AppendLine()
            receiptLines.AppendLine("Guest Name:")
            receiptLines.AppendLine("           _____________________________")
            receiptLines.AppendLine()
            receiptLines.AppendLine("Signature:")
            receiptLines.AppendLine("           _____________________________")
        Else
            receiptLines.AppendLine(DASH_LINE)
            receiptLines.AppendLine(String.Format("{0,-30} {1,13:N2}", "**Total USD**", grandTotal))
            receiptLines.AppendLine(DASH_LINE)

            Dim paid As Decimal : Decimal.TryParse(txtPaid.Text, paid)
            Dim khrRate As Decimal : Decimal.TryParse(txtKHRRate.Text, khrRate)
            Dim thbRate As Decimal : Decimal.TryParse(txtTHBRate.Text, thbRate)
            If khrRate <= 0 Then khrRate = 4100
            If thbRate <= 0 Then thbRate = 35

            Dim changeUSD As Decimal = Math.Max(0, paid - grandTotal)

            receiptLines.AppendLine(String.Format("{0,-15} {1,24}", "Amount", "Changes"))
            receiptLines.AppendLine(String.Format("USD {0,-11:N2} {1,24:N2}", paid, changeUSD))
            receiptLines.AppendLine(String.Format("KHR {0,-11:N0} {1,24:N0}", paid * khrRate, changeUSD * khrRate))
            receiptLines.AppendLine(String.Format("THB {0,-11:N0} {1,24:N0}", paid * thbRate, changeUSD * thbRate))

            receiptLines.AppendLine()
            receiptLines.AppendLine(String.Format("Cash  {0:N2}  {1}", grandTotal, DateTime.Now.ToString("dd-MM-yyyy hh:mmtt")))
            receiptLines.AppendLine("________________________________________")
            receiptLines.AppendLine(String.Format("Cashier: {0,-12} Close Time", Me.CashierName))
        End If
        ...
        receiptLines.AppendLine(vbCrLf & vbCrLf & vbCrLf)
        receiptLines.AppendLine(".")
    ...
    Private Sub lblCasheri_Click(sender As Object, e As EventArgs) Handles lblCasheri.Click
        lblCasheri.Text = CashierName
    End Sub

    Private Sub lblStroe_Click(sender As Object, e As EventArgs) Handles lblStroe.Click
        lblStroe.Text = StoreName
    End Sub

    Private Sub btnClostingReport_Click(sender As Object, e As EventArgs) Handles btnClostingReport.Click
    ...
    Private Sub btnOpenCashDrawer_Click(sender As Object, e As EventArgs) Handles btnOpenCashDrawer.Click
        ' NOTE: This relies on the OpenCashDrawer function in the PosPrinterCommands Module
        ' and the POS_PRINTER_NAME constant being set correctly.

        If String.IsNullOrEmpty(POS_PRINTER_NAME) Then
            MessageBox.Show("POS_PRINTER_NAME constant is not set.", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Exit Sub
        End If

        Dim overrideForm As New frm_OverridePin()
        overrideForm.StartPosition = FormStartPosition.CenterParent

        ' Open the PIN entry dialog
        If overrideForm.ShowDialog() = DialogResult.OK AndAlso overrideForm.IsPinValid Then
            ' User successfully authenticated (PIN is valid)
            Try
                ' The function OpenCashDrawer is in the PosPrinterCommands module.
                ' NOTE: You need to make sure 'PosPrinterCommands' module is available in your project.
                Dim success As Boolean = PosPrinterCommands.OpenCashDrawer(POS_PRINTER_NAME)

                If success Then
                    ' Drawers connected via the printer's RJ11 port will open here.
                    MessageBox.Show($"Cash drawer successfully opened by User ID: {overrideForm.OverrideUserId}.", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Information)
                Else
                    MessageBox.Show(
                    "Failed to send cash drawer command. Please verify:" & Environment.NewLine &
                    "1. The printer is powered on and connected." & Environment.NewLine &
                    "2. The printer name ('" & POS_PRINTER_NAME & "') is exact." & Environment.NewLine &
                    "3. The cash drawer is plugged into the printer's RJ11 port.",
                    V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
                End If

            Catch ex As Exception
                MessageBox.Show($"An unexpected error occurred while trying to open the cash drawer: {ex.Message}", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
            End Try
        Else
            ' PIN was invalid or the user canceled the override.
            MessageBox.Show("Cash drawer opening cancelled or authorization failed.", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Warning)
        End If
    End Sub

    Private Sub btnReciptReprint_Click(sender As Object, e As EventArgs) Handles btnReciptReprint.Click
        Dim F As New frm_reprint_recipt
        F.FormBorderStyle = FormBorderStyle.None
        F.ShowDialog()
    End Sub

    Private Sub btnPrintCheck_Click(sender As Object, e As EventArgs) Handles btnPrintCheck.Click
        ' 1. Validation: Ensure there is something to print
        If dg_Items.Rows.Count = 0 Then
            MessageBox.Show("No items found to print check.", V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Exit Sub
        End If

        ' 2. Update totals one last time to ensure accuracy
        UpdateTotalSummary()

        ' 3. Execute Printing with a placeholder Invoice Number
        ' We use "CHECK" or "PRE-CHECK" as the receipt number since it's not saved yet
        PrintReceiptAuto("CHECK-PRINT")

        ' Note: We DO NOT call btnClear_Click here because the user still needs to 
        ' process the actual payment (Settlement).
    End Sub
End Class